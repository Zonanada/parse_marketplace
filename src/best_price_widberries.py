from parse_link_wildberries import GetLinksWilberries
...
class BestPriceWildberries():
    ...
    def __init__(self, brand_name, type):
        self.type = type
        self.visited = dict()
        getLinksWildberries = GetLinksWilberries(brand_name, type)
        self.links = getLinksWildberries.get_links(self.visited)
        del getLinksWildberries
        print(len(self.links))
        if (len(self.links)):
            self.parse_pages(self.links)


    def parse_pages(self, links):
        parsePage = ParsePageWildberries(links, self.visited)
    ...
    def get_result(self):
        if (len(self.result) == 0):
            return "\nЛучшее предложение Wildberries:\nНет результата по вашему запросу!\nВозможно опечатка!"
        result = "\nЛучшее предложение Wildberries:\n"
        for index, url in enumerate(self.result):
            result += f"{index+1}) {self.data[url]['price_for_100_gramm']} рубл./100гр. {self.data[url]['name']}, {self.data[url]['price']} за {self.data[url]['grams']} грамм\n"
            result += f"Ссылка: {url}\n"
        return result
